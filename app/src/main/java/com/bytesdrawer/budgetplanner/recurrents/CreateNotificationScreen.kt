...
fun CreateNotificationScreen(
    ...
    val incomeExpenseState = remember { mutableStateOf(tabState != 0) }
    val expanded = remember { mutableStateOf(false) }
    val digitPattern = remember { Regex("^\\d+\\.?\\d*$") }
    val name = remember { mutableStateOf("") }
    val customMessage = remember { mutableStateOf("") }
    val transactionAmount = remember { mutableStateOf("") }
    val repeatingReminder = remember { mutableStateOf(1) }
    val optionalComment = remember { mutableStateOf("") }
    val frequency = remember { mutableStateOf(0) }
    ...
    val creatingNotification = remember {
        mutableStateOf(false)
    }
    ...
    LaunchedEffect(notifications) {
        if (creatingNotification.value) {
            CreateNotificationUtil.createAlarmToNotify(
                context,
                alarmMgr,
                notifications?.last()!!.notification_id,
        ...
        item(span = { GridItemSpan(5) }) {
            Column(
                ...
                Button(
                    ...
                    Text(text = stringResource(R.string.create_recurrent_button))
                }
